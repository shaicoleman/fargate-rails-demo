...
aws ec2 authorize-security-group-ingress --group-name fargate-web-app --protocol tcp --port 22 --cidr 0.0.0.0/0
...
fargatecli service create --port http:3000 fargate-web-app --security-group-id $security_group --image ../Dockerfile
./stop.sh
